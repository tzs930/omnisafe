...
def accumulate_replay_buffer(buffer, agent, epoch, max_trajectories: int = 1000, 
                           cost_threshold: float = 25.0, return_threshold: float = 0.0):
    ...
    if hasattr(buffer, 'buffers') and len(buffer.buffers) > 0:
        ...
        for env_idx, env_buffer in enumerate(buffer.buffers):
            env_data = env_buffer.data
            # print(f"  Processing environment {env_idx + 1}/{len(buffer.buffers)}")
            ...
            env_trajectories = _convert_to_trajectory_format(env_data, trajectory_ids)
            
            # Assign global trajectory IDs (0, 1, 2, ...) and remove env_id
            for traj in env_trajectories:
                traj['trajectory_id'] = global_trajectory_id
                if 'env_id' in traj:
        ...
        # Direct OnPolicyBuffer
        data = buffer.data
        # print("Accumulating data from OnPolicyBuffer")
        
        # Extract trajectory IDs
        trajectory_ids = _extract_trajectory_ids(data)
        
        # Convert to trajectory-unit format
        new_trajectories = _convert_to_trajectory_format(data, trajectory_ids)
    ...
    for traj in new_trajectories:
        ...
        if has_episode_end:
            ...
            print(f"  Discarding incomplete trajectory {traj['trajectory_id']} (no episode end)")
    ...
    print(f"Total trajectories (existing + new): {len(all_trajectories)}")
...
def save_replay_buffer(buffer, agent, epoch, max_trajectories: int = 1000, 
                      cost_threshold: float = 25.0, return_threshold: float = 0.0):
    ...
    stats = _calculate_trajectory_statistics(all_trajectories, cost_threshold, return_threshold)
...
def _extract_trajectory_ids(data):
    ...
    if hasattr(data, 'get'):
        ...
        timeout_flags = data.get('timeout', None)
    else:
    ...
    if terminals_flags is None and timeout_flags is None:
        ...
        data_length = len(data['obs']) if 'obs' in data else 1000
    ...
    # Remove incomplete trajectory (last one if it doesn't end with episode_end)
    if len(episode_ends) > 0 and not episode_ends[-1]:
        ...
        if len(last_complete_idx) > 0:
            ...
            # Truncate data to only include complete trajectories
            for key in data:
                if hasattr(data[key], '__len__') and len(data[key]) > last_complete_end:
                    data[key] = data[key][:last_complete_end]
            trajectory_ids = trajectory_ids[:last_complete_end]
            print(f"Removed incomplete trajectory, kept {last_complete_end} steps from {len(episode_ends)} total steps")
...
def _convert_to_trajectory_format(data, trajectory_ids):
    ...
    for i, traj_id in enumerate(trajectory_ids):
        ...
        for key in ['obs', 'next_obs', 'act', 'reward', 'cost', 'value_r', 'value_c', 'logp', 'terminals', 'timeout']:
            if key in data:
                value = data[key][i]
...
def _calculate_trajectory_statistics(trajectories, cost_threshold, return_threshold):
...
def main():
    ...
    algo = 'PPOLag'
    replay_buffer_save_freq = 10  # Save replay buffer every 10 epochs
    
    # Only override necessary configurations
    custom_cfgs = {
        'train_cfgs': {
            'total_steps': 1000000,  # Override for longer training
        },
        'algo_cfgs': {
            'steps_per_epoch': 5000,  # Reduced to avoid buffer overflow
        },
        'logger_cfgs': {
            'save_model_freq': 100,  # Override for more frequent model saving
        },
    }
    ...
    # Check if we have a replay buffer
    if not hasattr(algo_instance, '_buf'):
        print("No replay buffer found in algorithm instance")
        return
    
    print("Found replay buffer in algorithm instance")
    
    # Store original learn method
    original_learn = algo_instance.learn
